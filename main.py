...
def start():
    url_list = ['ttps''vietnamnet.vn']  #Chứa các đường link chưa duyệt
    history=[]  #Chứa các đường link đã duyệt
    max_page    #Quy định số lượng trang web được tải về
    ...
    while (count < max_page) and (len(url_list) > 0):
        ...
        for item in links:  #Duyệt từng đường link thu được để kiểm tra tính hợp lệ
            if web.kiem_tra_link(item):
                ...
                if (item not in url_list) and (item in history):    #Nếu đường link chưa được duyệt vào hàng đợi và chưa có trong lịch sử
